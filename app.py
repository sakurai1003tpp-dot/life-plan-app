...
import japanize_matplotlib  # グラフの日本語文字化けを防止する必須ライブラリ
...
with tab1:
    ...
    ax1.plot(
        age_history,
        cash_history,
        label=f"現預金 (保有上限 {max_cash_limit}万円)",
        color=COLOR_GREEN,
        linestyle="--",
        linewidth=2.0,
    )
...
with tab2:
    ...
    ax_g.axvline(retirement_age_h, color="#FF869E", linestyle=":", label="退職年齢")
    ...
    ax_g.set_ylabel("金額 (万円)", fontsize=11, color=COLOR_DARK)
    ...
    ax_n.set_ylabel("金額 (万円)", fontsize=11, color=COLOR_DARK)
...
with tab4:
    ...
    ax4.set_ylabel("割合 (%)", fontsize=11, color=COLOR_DARK)
    ax4.set_ylim(0, 100)
...
# ダウンロード用CSV作成（Shift-JISまたはBOM付きUTF-8推奨ですが、Excel向けにBOM付きUTF-8に設定）
